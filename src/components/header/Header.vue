    ...
    <header class="navbar navbar-expand navbar-dark flex-column flex-md-row bd-navbar">
        ...
        <ul class="navbar-nav flex-row ml-md-auto d-none d-md-flex">
            ...
            <a class="nav-item nav-link dropdown-toggle mr-md-2" href="#" 
                @click="check">
                CART <span class="badge badge-danger">{{this.carts.length ? this.carts.length : 0}}</span>
            </a>
            <div :class="'dropdown-menu dropdown-menu-right ' + (flag ? 'show' : '')" aria-labelledby="bd-versions">
                <a class="dropdown-item active" href="/docs/4.0/">product active</a>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="#"
                    v-for="(cart, index) in carts" :key="index">{{cart.name}}</a>
